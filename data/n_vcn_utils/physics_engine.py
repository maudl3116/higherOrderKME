# code from https://github.com/pairlab/v-cdn.git
...
class BallEngine(Engine):
    ...
    def add_boundary_impulse(self, p_range=(-75, 75, -75, 75)):
        ...
        for i in range(self.n_ball):
            ...
            self.balls[i].apply_impulse_at_local_point(impulse=(impulse[0],impulse[1]), point=(0, 0))
    ...
    def add_action(self, action):
        if action is None:
            return
        for i in range(self.n_ball):
            self.balls[i].apply_force_at_local_point(force=(action[i][0],action[i][1]), point=(0, 0))
